DROP DATABASE IF EXISTS energy_project;
...
CREATE TABLE Load_ (
    ...
    # Type of the load (e.g., "energy", "water", "gas").
    demand DECIMAL(10, 2),
    # Demand value for the load.           
    site_id INT,
    FOREIGN KEY (site_id) REFERENCES Site(id)
);
-- Table for Utility
...
CREATE TABLE HydrogenFuelCell (
    ...
    power_output_curve JSON,
    -- Using JSON to store complex data like DataFrame or Dict
...
CREATE TABLE HydrogenFuelCellPlant_HydrogenFuelCell (
    ...
    hydrogen_fuel_cell_id INT,
    PRIMARY KEY (
        hydrogen_fuel_cell_plant_id,
        hydrogen_fuel_cell_id
    ),
...
CREATE TABLE Biomass (
    ...
    feedstock_characteristics JSON,
    -- Using JSON to store complex data like Dict
    emission_rate DECIMAL(10, 2),
    renewable_sources VARCHAR(255)
);
...
CREATE TABLE PVArray (
    ...
    module_parameters JSON,
    -- Using JSON to store complex data like Dict or pd.Series
    temperature_model_parameters JSON,
    -- Using JSON to store complex data like Dict or pd.Series
    capacity DECIMAL(10, 2),
    lifespan INT,
    DC_AC_Out VARCHAR(255),
